# Claude Code Prompt v4 — Redefine Layout Panel and Fix Layout Bugs in `FlightDataDashboard.m`
...
# 28. v4 Stability Addendum — Resize, Splitter Limits, Logging, and Implementation Discipline

This v4 addendum strengthens the prompt against common MATLAB GUI regression risks.

The goal is not to add new features. The goal is to prevent edge-case regressions while correcting the layout architecture.

---

## 28.1 Resize / SizeChangedFcn Safety

The layout normalization logic must be safe during figure resize events.

When the user resizes the main window, or when `SizeChangedFcn` triggers layout reflow:

```text
plot/dataView column must remain '1x' whenever plot/dataView is visible.
resize-driven reflow must not convert plot/dataView into a fixed numeric width.
resize-driven reflow must not restore stale UserColumnWidths that contain plot as numeric.
resize-driven reflow must not open Video Player.
resize-driven reflow must not toggle PanelVisible.
resize-driven reflow must not change BoardOffState.
```

If existing code calls `applyResponsiveLayout`, `onFigureSizeChanged`, `reflowBoardColumns`, or any layout helper from `SizeChangedFcn`, ensure that path ends with the same safe normalization rule:

```text
plot visible => plot column = '1x'
```

The normalization helper must remain idempotent under repeated resize events.

Repeated window resize must not cause progressive drift, such as:

```text
panel widths gradually shrinking
splitters moving unexpectedly
plot becoming fixed width
hidden columns reappearing
board row ratio drifting
```

---

## 28.2 Splitter Minimum Width and Clamp Rules

Splitter drag must clamp adjustable fixed-width panels to reasonable limits before saving widths.

Recommended minimum widths:

```text
infoWidth >= 100 px unless info is explicitly hidden
attitudeWidth >= 80 px unless attitude is explicitly hidden
mapAltWidth >= 120 px unless both mapOnly and altOnly are explicitly hidden
```

Exact values may be adjusted to fit the current UI, but the implementation must prevent:

```text
negative widths
zero-width panels caused only by drag
sub-pixel / near-zero widths
panels collapsing unintentionally
splitters crossing over adjacent panels
saved UserColumnWidths that make the layout unrecoverable
```

Splitter drag rules:

```text
Clamp before assigning to dataGrid.ColumnWidth.
Clamp before saving to UserColumnWidths.
Do not save invalid widths.
Do not save hidden panel widths as active user widths.
Do not save plot/dataView width as numeric.
```

If a user wants a panel hidden, that must happen through the panel toggle button, not by dragging a splitter past zero.

---

## 28.3 Preserve Existing Try-Catch and Logging Patterns

Do not remove robust error handling while refactoring layout code.

Preserve existing patterns such as:

```text
try / catch
app.logCaught(ME, 'tag')
safe validity checks with isempty / isvalid / isfield
```

When moving or splitting layout helpers:

```text
keep equivalent try-catch protection
keep equivalent app.logCaught diagnostic logging
do not replace robust handling with silent catch
do not remove safety guards around UI handles
```

If a new layout helper is added, it should follow the existing defensive style:

```text
validate app.UI existence
validate fIdx range
validate UI handles with isfield / isempty / isvalid
log caught errors with a meaningful tag
return safely on invalid UI state
```

---

## 28.4 Stronger Pre-Edit Verification Mandate

Before any edit, Claude Code must first search the current file for the following symbols and understand their actual usage:
...
Before implementing `normalizeColumnWidthsForVisiblePanels`, Claude Code must identify the actual column index meaning in the current source.

Do not assume column indices from previous reports.

The implementation must be based on the current file’s real `dataGrid` construction and `ColumnWidth` usage.

Minimum verification before editing:

```text
Find where dataGrid is created.
Find its ColumnWidth initial value.
Find which UI component is assigned to each column.
Find all splitters and their column indices.
Find where ColumnWidth is changed after creation.
Find where UserColumnWidths is saved/restored.
Find all SizeChangedFcn/reflow paths.
```

Only after this mapping is understood should the normalization helper be implemented.

---

## 28.5 Board-Off Implementation Strategy: Preferred and Fallback

Board-off must implement active single-board analysis arrangement.

## Preferred strategy

Reuse the active board’s existing `dataGrid` / panel containers as much as possible.
...
```text
the active board’s own panels appear rearranged
no unrelated summary panel appears
no Video Player opens
no extreme board-height-only expansion
```

## Fallback strategy

If fully removing or replacing `boardOffPanel` is too risky in one patch:

```text
Keep boardOffPanel only as a compatibility container.
Replace its content so it visually represents the active source board's:
    info + plot
    plus remaining visible panels
Do not let it behave as an unrelated summary panel.
Do not let it duplicate stale or inconsistent content.
Do not open Video Player.
```

The fallback is acceptable only if the visible behavior satisfies the required board-off arrangement.

The fallback must still pass the acceptance criteria for:
...
no video popup
upper info+plot
lower remaining panels
stable reset
stable project round-trip
```

---

## 28.6 Reset Must Be Narrow and Non-Destructive

Reset / Auto Fit must be narrowly defined.

Reset may only:

```text
clear layout width cache
clear arrangement-specific temporary width ratios
recompute default internal arrangement
normalize ColumnWidth
restore plot/dataView to '1x' when visible
```
...
open Video Player
close Video Player
force a focus preset
change BodyGrid.RowHeight when both boards are visible
change BodyRowSplitRatio when both boards are visible
discard loaded flight/video data
change selected row/current index
reset sync state
```

Reset is a layout geometry recovery action, not an application state reset.

---

## 28.7 Section 27 Should Be Treated as Implementation Guidelines

The detailed `27.x` sections are intentionally verbose to prevent mistakes.

Do not compress them away during implementation.

Treat them as mandatory implementation guidelines.

If creating an internal task plan, group them under:

```text
Implementation Guidelines and Mandatory Checks
```

but preserve the substance of each rule.

The implementation must still satisfy:

```text
screenshot order rule
pre-edit search mandate
board-off active panel rearrangement
column normalization safety
resize safety
splitter min-width clamp
try-catch/logging preservation
strict fix order
narrow reset definition
test hook false-success prevention
repeated operation stability
```

---

## 28.8 Additional Regression Tests Required for v4

Add or manually verify these additional cases beyond the previous G-LAYOUT tests.

```text
G-LAYOUT-11: Resize after splitter drag preserves plot = '1x'.
G-LAYOUT-12: Resize after layout preset preserves PanelVisible and BoardOffState.
G-LAYOUT-13: Resize after board-off preserves upper info+plot / lower remaining panels.
G-LAYOUT-14: Splitter drag clamps info width to minimum and does not save invalid width.
G-LAYOUT-15: Splitter double-click reset clears only relevant width cache.
G-LAYOUT-16: Reset preserves PanelVisible, BoardOffState, Video Player state, selected row, and loaded data.
G-LAYOUT-17: Project load with legacy corrupted numeric plot width normalizes plot back to '1x'.
G-LAYOUT-18: Repeated resize/layout/splitter cycles do not cause progressive layout drift.
```

If full GUI automation is difficult, expose deterministic test hooks for:

```text
current normalized ColumnWidth
stored UserColumnWidths fields
actual vidViewerDialog visible state
current BodyGrid.RowHeight
current BodyRowSplitRatio
current PanelVisible
active board-off source
current arrangement preset
```

---

## 28.9 v4 Final Architecture Reminder

The final code architecture must keep these responsibilities separated:

```text
Layout buttons:
    arrange currently visible panels only

Panel buttons:
    show/hide panels only

Board-off buttons:
    switch to single-board analysis arrangement only

Video controls:
    open/close external Video Player only

Splitters:
    adjust fixed panel widths only

Plot/dataView:
    always remains flex '1x' when visible

Project save/load:
    stores only valid layout state and normalizes on restore

Resize:
    recomputes geometry but preserves state and plot flex
```

Any code path that violates this separation must be refactored, guarded, or explicitly isolated.
