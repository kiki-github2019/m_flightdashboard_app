...
- `상단 보드 off`일 때 off-mode 패널은 Flight Data 2의 `현재 비행 정보`와 `데이터 뷰 패널`을 표시하고, Flight Data 2 원본 보드에서는 해당 두 패널 열만 숨긴다.
- `하단 보드 off`일 때 off-mode 패널은 Flight Data 1의 `현재 비행 정보`와 `데이터 뷰 패널`을 표시하고, Flight Data 1 원본 보드에서는 해당 두 패널 열만 숨긴다.
