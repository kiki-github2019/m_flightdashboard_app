...
% Markdown report under the "static check" folder.
    ...
    addBlockAllPatternCheck('openVideoReader', 'AVI-002', ...
        'AVI load updates both runtime path fields', ...
        {'VideoFilePath\s*\{fIdx\}', 'Models\s*\(fIdx\)\.aviFilePath'});
    addBlockAnyPatternCheck('UIFigureCloseRequest', 'CLOSE-001', ...
        'CloseRequest handles dirty project/options before delete', ...
        {'ProjectDirty', 'saveProjectFile\s*\(', 'writeOptionFileAtomic\s*\('});
    ...
    addBlockPatternCheck('collectPathPairs', 'EXPORT-003', ...
        'Export verification uses rewritten project paths, not copyMap only', ...
        'function\s+pairs\s*=\s*collectPathPairs\s*\(\s*~\s*,\s*~\s*,\s*copyMap\s*\)', false);
    addBlockPatternCheck('rebuildPlotsFromConfig', 'PLOT-001', ...
        'PlotConfig rebuild avoids default-tab index drift', ...
        'clearAllTabs\s*\([^\)]*\)[\s\S]*addPlotTab\s*\(', false);
    addBlockPatternCheck('capturePlotConfigFromUi', 'PLOT-002', ...
