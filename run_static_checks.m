...
% Markdown report under the "static check" folder.
    ...
    addBlockAllPatternCheck('openEditDialog', 'UI-002', ...
        'Edit dialog uifigure is created and stored', ...
        {'fig\s*=\s*uifigure\s*\(', 'EditDialog\s*=\s*fig'});
    addBlockAllPatternCheck('FlightDataDashboard', 'UI-003', ...
        'UIFigure disables AutoResizeChildren before SizeChangedFcn', ...
        {'AutoResizeChildren', 'SizeChangedFcn'});
    addPatternCheck('UI-004', 'Main callback methods are publicly accessible', ...
        'methods\s*\(Access\s*=\s*public\)[\s\S]*function\s+handleFlightFile', true);
    addPatternCheck('UI-005', 'Layout callback methods are publicly accessible', ...
        'methods\s*\(Access\s*=\s*public\)[\s\S]*function\s+toggleBoardVisibility', true);
    ...
    addBlockAllPatternCheck('refreshBoardOffSummaryPanel', 'BOARD-007', ...
        'Board off summary displays the opposite board info and data-view panel', ...
        {'sourceIdx\s*=\s*app\.getBoardOffSourceIdx\s*\(fIdx\)', ...
         'UI\s*\(sourceIdx\)\.dataTable\.Data', 'rebuildBoardOffPlots\s*\(fIdx,\s*sourceIdx\)'});
    addBlockAllPatternCheck('toggleBoardVisibility', 'BOARD-008', ...
        'Source board hides moved info and data-view columns while preserving other panels', ...
        {'captureBoardPanelState\s*\(sourceIdx\)', '(hideBoardInfoPlotColumns|reflowBoardColumns)\s*\(sourceIdx\)'});
    addBlockAllPatternCheck('hideBoardInfoPlotColumns', 'BOARD-009', ...
        'Moved source columns are hidden without changing other panel columns', ...
        {'widths\s*\{3\}\s*=\s*0', 'widths\s*\{4\}\s*=\s*0', 'widths\s*\{5\}\s*=\s*0'});
    addBlockAllPatternCheck('rebuildBoardOffPlots', 'BOARD-010', ...
        'Board off summary plot markers keep drag callbacks wired to the source board', ...
        {'HitTest''\s*,\s*''on', 'startPlotMarkerDrag\s*\(sourceIdx,\s*tIdx'});
    addBlockAllPatternCheck('createBoardOffSummaryPanel', 'BOARD-011', ...
        'Board off summary data-view panel keeps tab controls and table plot context menu', ...
        {'boardOffAddPlotTab\s*\(fIdx\)', 'boardOffClearCurrentTab\s*\(fIdx\)', ...
         'boardOffPlotSelectedVariable\s*\(fIdx\)', 'boardOffTableSelection\s*\(fIdx'});
