...
function i_applyAction(app, act, beforeState, outDir, caseIdx, stepIdx, captureOpts)
    switch act.fn
        ...
        case 'assertPath3DRenderState'
            % [#6] strengthen past-trajectory render verification using exposed getPath3DState fields
            fIdx = act.args{1};
        ...
        case 'saveLoadPath3DProjectRoundTrip'
            fIdx = act.args{1};
            projectPath = [tempname() '.fdproj'];
            i_tempProjectFileRegistry('add', projectPath);
            okSave = app.testHook('saveProjectFile', projectPath);
            if ~okSave || ~isfile(projectPath)
                error('AutoTest:Path3DProjectRoundTrip', '3D Path project save failed: %s', projectPath);
            end
            app.testHook('setPath3DDialogVisible', fIdx, false);
            app.testHook('autoLoadProjectFromFile', projectPath);
            ps = app.testHook('getPath3DState');
            if ~ps.desiredVisible(fIdx)
                error('AutoTest:Path3DProjectRoundTrip', 'board %d 3D Path desired state was not restored from disk project', fIdx);
            end
            stAfterLoad = app.testHook('getTestState');
            if any(stAfterLoad.BoardOffState)
                if ps.visible(fIdx)
                    error('AutoTest:Path3DProjectRoundTrip', 'board %d 3D Path visible while board-off active after project load', fIdx);
                end
            elseif ~ps.visible(fIdx)
                error('AutoTest:Path3DProjectRoundTrip', 'board %d 3D Path dialog was not restored from disk project', fIdx);
            end
...
function cases = i_buildCaseMatrix()
    ...
    P3DS = @(fIdx, lbl)        struct('fn','saveLoadPath3DProjectRoundTrip','args',{{fIdx}},      'label',lbl, 'row',NaN);
    P3DR = @(fIdx, lbl)        struct('fn','assertPath3DRenderState',       'args',{{fIdx}},      'label',lbl, 'row',NaN);
    ...
    cases(end + 1) = mk('J-PANEL-SYNC', 'J-PANEL-SYNC-31 dual video control sync independence (move F1)', ...
        ...
        {BVR('reset board-off'), SVS(1, 1, 0, 30, 50, 'sync F1'), SVS(2, 1, 0, 30, 50, 'sync F2'), ...
         GTF(1, 12, 'F1 go to frame 12')});
    cases(end + 1) = mk('J-PANEL-SYNC', 'J-PANEL-SYNC-32 dual video control sync independence (move F2)', ...
    ...
    % [#1] placed last so K-PANEL-CAPTURE (and all earlier categories) keep their case numbers.
    ...
    cases(end + 1) = mk('K-PATH3D', 'K-PATH3D-05 3D Path follows time changes', ...
        ...
        {BVR('reset board-off'), P3D(1, true, 'open F1 3D Path'), ATC(1, 10, 'move F1 time'), ...
         ATC(1, 20, 'move F1 time again'), ...
         struct('fn', 'assertPath3DRenderState', 'args', {{1}}, 'label', 'verify F1 3D render state', 'row', NaN), ...
         P3D(1, false, 'close F1 3D Path')});
    ...
    cases(end + 1) = mk('K-PATH3D', 'K-PATH3D-07 Flight 2 open/time render', ...
        '3D Path Flight 2 render', 'Flight 2 dialog renders full/past trajectory after time changes', ...
        {BVR('reset board-off'), P3D(2, true, 'open F2 3D Path'), ATC(2, 10, 'move F2 time'), ...
         ATC(2, 20, 'move F2 time again'), P3DR(2, 'verify F2 3D render state'), ...
         P3D(2, false, 'close F2 3D Path')});
    cases(end + 1) = mk('K-PATH3D', 'K-PATH3D-08 visible F2 hides/restores on upper board-off', ...
        '3D Path board-off restore', 'visible F2 dialog hides during upper board-off and restores on board-on', ...
        {BVR('reset board-off'), P3D(2, true, 'open F2 3D Path'), BV(1, 'upper board off'), ...
         BV(1, 'upper board on'), P3DR(2, 'verify F2 3D render state after restore'), ...
         P3D(2, false, 'close F2 3D Path')});
    cases(end + 1) = mk('K-PATH3D', 'K-PATH3D-09 FlightPlay row move updates 3D Path', ...
        '3D Path flight-play sync', 'flight play row movement keeps 3D path render state valid', ...
        {BVR('reset board-off'), P3D(1, true, 'open F1 3D Path'), FPT(1, 'open F1 flight play'), ...
         FPM(1, 10, 'F1 +10 through flight play'), FPR(1, 'refresh F1 flight play'), ...
         P3DR(1, 'verify F1 3D render state after flight play'), FPT(1, 'close F1 flight play'), ...
         P3D(1, false, 'close F1 3D Path')});
    cases(end + 1) = mk('K-PATH3D', 'K-PATH3D-10 disk project save/load round-trip', ...
        '3D Path disk project state', 'Path3DVisible survives saveProjectFile + autoLoadProjectFromFile', ...
        {BVR('reset board-off'), P3D(1, true, 'open F1 3D Path'), ...
         P3DS(1, 'save/load disk project with F1 3D visible'), ...
         P3D(1, false, 'close F1 3D Path')});
    cases(end + 1) = mk('K-PATH3D', 'K-PATH3D-11 armed 3D Path disk round-trip while board-off', ...
        '3D Path disk project board-off', 'armed-but-hidden Path3DVisible survives disk save/load under board-off', ...
